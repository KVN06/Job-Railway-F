import { createTraining } from '../api/trainings.js';
...
export async function mountAdminTrainingCreate(root){
  const container = root || document.querySelector('[data-page="admin-training-create"]');
  if (!container) return;
  const { allowed } = await requireRole('admin');
  if (!allowed) return;
  container.innerHTML = '';
  container.appendChild(h('h1',{class:'text-2xl font-bold text-gray-800 mb-6'},'Crear Nueva Capacitación'));
  ...
  const btnSubmit = h('button',{type:'submit',class:'bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded'},'Crear Capacitación');
  ...
  form.addEventListener('submit', async (ev)=>{
    ...
    try {
      ...
      u.searchParams.set('success','Capacitación creada correctamente');
      location.href = u.toString();
    } catch(err){
      alert('No se pudo crear: ' + (err?.data?.message || err?.message || 'Error'));
    } finally { btnSubmit.disabled = false; }
  });
}

export function autoMount(){ const host=document.querySelector('[data-page="admin-training-create"]'); if(host) mountAdminTrainingCreate(host); }
